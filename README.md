...
2.  **执行程序**
    - 在项目根目录下，通过命令行运行 `main.py`，并传入教授姓名作为参数。

    - **分析真实数据**:
      使用 `--no-test-mode` 标志来指定程序使用 `data/` 目录下的真实数据。
      ```bash
      # 示例：分析 "王剑威" 教授
      python main.py "王剑威" --no-test-mode
      ```

    - **运行测试模式**:
      如果不加 `--no-test-mode` 标志，程序将默认在测试模式下运行，使用 `data/sample/` 目录中的数据。
      ```bash
      # 示例：以测试模式运行
      python main.py "测试教授"
      ```
