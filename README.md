...
1.  **准备数据**
    - 在 `data/` 目录下，以目标教授的姓名创建一个新的文件夹，例如 `data/王教授/`。
    - 在新创建的教授文件夹内，建立三个子目录：`main`, `ref1`, `ref2`。
      - `main/`: 存放教授的**核心代表作**。
      - `ref1/`: 存放用于分析**领域热点**的关键参考文献。
      - `ref2/`: 存放用于启发**本科生项目**的复杂度适中的论文。
    - 将整理好的论文（`.md` 格式）放入对应的文件夹中。
    - *（可选）* 如果想快速测试，可以直接使用 `data/sample/` 中的数据，无需创建新目录。

2.  **配置主程序**
    - 打开 `main.py` 文件。
    - 找到文件底部的 `if __name__ == "__main__":` 部分。
    - 修改 `main()` 函数的参数：
      - **分析新数据**: 设置 `professor_name="王教授"` (与你在 `data` 目录中创建的文件夹同名)，并设置 `test_mode=False`。
      - **运行测试模式**: 保持 `professor_name="测试教授"` 和 `test_mode=True`。
...
- 程序执行完毕后，一份详细的 Markdown 格式分析报告将保存在 `output/` 目录下，文件名类似于 `王教授_final_report.md`。
- 包含所有工作流产出的结构化数据的 JSON 文件也会保存在 `output/` 目录下，文件名类似于 `王教授_research_data.json`，供进一步分析使用。
